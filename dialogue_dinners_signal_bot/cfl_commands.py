...
class CFLCommands(Command):
    ...
    async def _list_pairs(self, pairs, c: Context):
        ...
        await c.edit(response, None, list_pairs)
    ...
    async def handle(self, c: Context):
        ...
        if c.message.text.startswith("!dd") and c.message.source_number == os.environ["PHONE_NUMBER"]:
            ...
            if command == "test":
                await c.edit("Pairings are working!")
            elif command == "poll":
                await c.edit("If you are interested in participating in dialogue dinners this semester, please react below.")
            elif command == "add":
                user_list = c.message.mentions
                self.db_service.add(user_list)
                await c.edit("Users added")
                ...
                await c.edit("Self added to the database.")
            elif command == "pairs":
                ...
                if pairs:
                    ...
                    await c.edit("Failed to list pairs or no active pairings found.")
                ...
                if pairs:
                    ...
                    await c.edit("Failed to generate pairs or no pairs to generate!")
            elif command == "regenerate":
                pairs = self.db_service.regenerate()
                if pairs:
                    await self._list_pairs(pairs, c)
                else:
                    await c.edit("Failed to regenerate pairs or no pairs to regenerate!")
            elif command == "complete":
                # Complete a pairing
                user_list = c.message.mentions 
                if self.db_service.complete(user_list[0], user_list[1]):
                    await c.edit(f"Pairing completed.")
                else: 
                    await c.edit("Failed to complete pairing. Make sure the users are paired and active.")
                ...
                if self.db_service.complete(user_list[0], {"uuid": c.message.source_uuid}):
                    await c.edit(f"Pairing completed with self.")
                else: 
                    await c.edit("Failed to complete pairing with self. Make sure the users are paired and active.")
            elif command == "reset":
                # Reset the database
                self.db_service.reset()
                await c.edit("Database reset.")
            elif command == "help":
                help_message = (
                    "Available commands:\n"
                ...
                await c.edit(help_message)
            else:
                await c.edit("Unknown command. Use !dd help for a list of commands.")
